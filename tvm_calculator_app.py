...
with tab_single:
    ...
    if err:
        ...
        flows = {}
        if solve_for == "Future Value (FV)":
            st.metric("Future Value (FV)", fmt_money(result))
            ...
            st.metric("Present Value (PV)", fmt_money(result))
...
with tab_annuity:
    ...
    if err:
        ...
        if solve_for == "Future Value (FV)":
            ...
            st.metric("Payment (PMT)" + (" — first payment" if is_growing else ""), fmt_money(result))
...
with tab_perp:
    ...
    if err_p:
        ...
        if solve_for_p == "Present Value (PV)":
            st.metric("Present Value (PV)", fmt_money(result_p))
            ...
            st.metric("Payment (PMT)", fmt_money(result_p))
...
with tab_mortgage:
    ...
    st.write("Enter loan terms to see your payment, a full amortization schedule, and how extra payments accelerate payoff.")

    mc1, mc2, mc3 = st.columns(3)
    with mc1:
        loan_amount = st.number_input("Loan Amount", value=350000.0, step=5000.0, key="mtg_amt")
    ...
    r1.metric("Scheduled Payment", fmt_money(summary_m["scheduled_payment"]))
    r2.metric("Total Interest Paid", fmt_money(summary_m["total_interest"]))
    ...
    with d2:
        summary_rows_m = [
            ("Loan Amount", fmt_money(loan_amount)), ("Annual Rate", fmt_pct(annual_rate_m / 100)),
            ("Term (years)", f"{years_m:.0f}"), ("Payments per Year", ppy_m),
            ("Extra Payment", fmt_money(extra_pmt)), ("Scheduled Payment", fmt_money(summary_m["scheduled_payment"])),
            ("Total Interest", fmt_money(summary_m["total_interest"])), ("Payoff Time (years)", f"{summary_m['payoff_years']:.2f}"),
        ]
        excel_buf_m = build_amortization_excel("Mortgage / Loan Amortization Schedule", summary_rows_m, schedule_m)
        st.download_button("Excel (.xlsx)", data=excel_buf_m, file_name="amortization_schedule.xlsx",
                            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)

    pdf_table_rows = [[r["period"], fmt_money(r["payment"]), fmt_money(r["interest"]), fmt_money(r["principal"]), fmt_money(r["balance"])]
                       for r in schedule_m]
    pdf_buf_m = build_simple_pdf(
        "Mortgage / Loan Amortization Schedule", f"Loan: {fmt_money(loan_amount)} at {annual_rate_m:.2f}% for {years_m:.0f} years",
        summary_rows_m, ["Period", "Payment", "Interest", "Principal", "Balance"], pdf_table_rows,
...
with tab_retire:
    ...
    if n_years <= 0:
        ...
        m1.metric("Projected Balance at Retirement (nominal $)", fmt_money(nominal_fv))
        m2.metric("Same Balance in Today's Dollars (real)", fmt_money(real_fv))
        m3.metric("Target Needed (nominal $ at retirement)", fmt_money(target_nominal))
        ...
        with d2:
            summary_rows_r = [
                ...
                ("Current Savings", fmt_money(current_savings)), ("Annual Contribution (Year 1)", fmt_money(annual_contribution)),
                ("Contribution Growth Rate", fmt_pct(g_r)), ("Investment Return", fmt_pct(i_r)),
                ("Inflation Rate", fmt_pct(infl_r)), ("Projected Nominal Balance", fmt_money(nominal_fv)),
                ("Projected Real (Today's $) Balance", fmt_money(real_fv)), ("Target (nominal $ at retirement)", fmt_money(target_nominal)),
            ]
        ...
        pdf_rows_r = [[int(r["age"]), fmt_money(r["contribution"]), fmt_money(r["balance"]), fmt_money(r["real_balance"])] for r in df_r.to_dict("records")]
...
with tab_lottery:
    ...
    st.divider()
    m1, m2 = st.columns(2)
    m1.metric(f"PV of Annuity Option{' (after tax)' if apply_tax else ''}", fmt_money(pv_annuity_after))
    m2.metric(f"Lump Sum{' (after tax)' if apply_tax else ''}", fmt_money(lump_after))
    ...
    summary_rows_l = [
        ("First Payment", fmt_money(first_payment)), ("Number of Payments", int(n_payments)),
        ("Payment Growth Rate", fmt_pct(payment_growth / 100)), ("Discount Rate", fmt_pct(discount_rate_l / 100)),
        ("PV of Annuity Option", fmt_money(pv_annuity_opt)), ("Lump Sum Offered", fmt_money(lump_sum)),
        ("Break-even Discount Rate", fmt_pct(breakeven) if breakeven else "N/A"),
    ]
    if apply_tax:
        summary_rows_l += [("Tax Rate Applied", fmt_pct(tax_rate / 100)),
                            ("PV of Annuity (after tax)", fmt_money(pv_annuity_after)),
                            ("Lump Sum (after tax)", fmt_money(lump_after))]
...
with tab_cc:
    ...
    m3.metric(f"Balance after {cc_months} months (unpaid)", fmt_money(cc_fv))
    ...
    summary_rows_cc = [
        ...
        ("True EAR", fmt_pct(cc_ear)), ("Starting Balance", fmt_money(cc_balance)),
        ("Months Projected", cc_months), (f"Balance After {cc_months} Months", fmt_money(cc_fv)),
        ("Interest Accrued", fmt_money(cc_fv - cc_balance)),
    ]
