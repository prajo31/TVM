...
with tab_mortgage:
    ...
    if loan_amount <= 0:
        st.error("Loan amount is $0 — the down payment covers the full home price, so there's nothing to amortize.")
    else:
        ...
        pdf_buf_m = build_simple_pdf(
            ...
            f"Home Price: {fmt_money_plain(home_price)}  |  Down: {fmt_money_plain(down_payment)} ({down_pct_in:.1f}%)  |  "
